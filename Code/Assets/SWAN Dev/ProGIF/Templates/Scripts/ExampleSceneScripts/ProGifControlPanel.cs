...
public class ProGifControlPanel : MonoBehaviour
{
	...
	{
        // Use the recorder settings in the GIF manager, or assign your variables
        int width = ProGifManager.Instance.m_Width;
        int height = ProGifManager.Instance.m_Height;
        int quality = ProGifManager.Instance.m_Quality;
        int loop = ProGifManager.Instance.m_Loop;
        ...
        Close(()=>{
			...
			{
				ProGifManager.Instance.SetRecordSettings(_GetAspectRatio(dropdown_AspectRatio.value), width, height, slider_Duration.value, (int)slider_FPS.value, loop, quality);
			}
			else
            {
                ProGifManager.Instance.SetRecordSettings(true, width, height, slider_Duration.value, (int)slider_FPS.value, loop, quality);
            }
