...
namespace Game
{
    public class DamageHelper
    {
        ...
        {
            return CalcDamage(attcher, enemy, skill, null);
        }

        public static DamageResult CalcDamage(AttributeBonus attcher, AttributeBonus enemy, SkillPanel skill, SkillPanel from)
        {
            //计算公式  ((攻击 - 防御) * 百分比系数 + 固定数值) * 暴击?.暴击倍率 * (伤害加成-伤害减免) * (幸运)
            ...
            long AttrIncrea = skill.AttrIncrea;
            if (from != null)
            {
                AttrIncrea = (long)(AttrIncrea * (1 + from.AttrIncrea / 100.0));
                //AttrIncrea += from.AttrIncrea;
            }

            roleAttr = roleAttr * (100 + AttrIncrea + attcher.GetAttackAttr(AttributeEnum.AurasAttrIncrea)) / 100;  //职业攻击
            ...
            long ignoreDef = skill.IgnoreDef + attcher.GetAttackAttr(AttributeEnum.DefIgnore);
            if (from != null)
            {
                ignoreDef += from.IgnoreDef;
            }
            ignoreDef = Math.Min(ignoreDef, 100);
            def = def * (100 - ignoreDef) / 100;
            ...
            //最终伤害加成
            long FinalIncrea = skill.FinalIncrea;
            if (from != null)
            {
                //FinalIncrea += from.FinalIncrea;
                FinalIncrea = (long)(FinalIncrea * (1 + from.FinalIncrea / 100.0));
            }
            lg.Mul((100 + FinalIncrea) / 100.0);
            ...
            LargeNumber extendDamageLg = new LargeNumber(0);
            double at = attcher.GetAttackDoubleAttr(AttributeEnum.RealHpDamage);
            if (at > 0)
            {
    ...
    public class DamageResult
    {
        public DamageResult(LargeNumber damage, LargeNumber extendDamage, MsgType type, RoleType roleType, int skillId)
        {
            ...
            this.ExtendDamageLg = extendDamage;
            this.Type = type;
        ...
        public LargeNumber ExtendDamageLg { get; set; }
