...
public class Panel_Exclusive_Up_Dark : MonoBehaviour
{
    ...
    private const int StartPosition = 1013;
    ...
    void Start()
    {
        ...
        ExclusiveDevourConfig config = ExclusiveDevourConfigCategory.Instance.GetByCycleAndLevel(3, 1);
    ...
    private void Load()
    {
        ...
        for (int BoxId = 0; BoxId < MaxMain; BoxId++)
        {
            ...
            if (exclusive.GetQuality() < 8 || exclusive.GetLayer() <= 2)
            {
                continue;
            }
