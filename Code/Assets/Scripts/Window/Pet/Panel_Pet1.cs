...
public class Panel_Pet1 : MonoBehaviour, IBattleLife
{
    ...
    {
        GameProcessor.Inst.EventCenter.AddListener<Pet1BattleDownEvent>(this.PetBattleDown);
        GameProcessor.Inst.EventCenter.AddListener<OpenPetForgeEvent>(this.OpenPetForge);
    }

    private void PetBattleDown(Pet1BattleDownEvent e)
    {
    ...
    public void Show()
    {
        ...
        List<Pet> pets = user.PetDict.Select(m => m.Value).ToList();
