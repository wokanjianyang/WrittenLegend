...
public class Map_Dialog_Babel_Myth : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            GameProcessor.Inst.Babel_Myth_Auto = isOn;
        });
    ...
    private void Show()
    {
        ...
        long progress = user.BabelMythData.Data;
        ...
        if (progress == 0 && user.BabelMythCount.Data == 0)
        {
        ...
        Txt_Count.text = "今日挑战次数:" + user.BabelMythCount.Data;

        if (nextProgress > ConfigHelper.BabelMythMax)
        {
            Txt_Reward.text = "已通关，等待开发上限";
        }
        else
        {
            BabelMythConfig rewardConfig = BabelMythConfigCategory.Instance.GetByProgress(nextProgress);
